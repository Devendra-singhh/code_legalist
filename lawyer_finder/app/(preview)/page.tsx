...
export default function Chat() {
  ...
  const currentToolCall = useMemo(() => {
    ...
    if (lastMessage.function_call) {
      ...
      return {
        name: typeof functionCall === 'string' ? '' : functionCall.name || '',
        args: typeof functionCall === 'string' ? {} : JSON.parse(functionCall.arguments || '{}')
      };
    }
    return undefined;
  }, [messages]);
...
const Loading = ({ tool }: { tool?: { name: string; args: any } }) => {
  const toolName =
    tool?.name === "getInformation"
      ? "Getting information"
      : tool?.name === "addResource"
        ? "Adding information"
        : "Thinking";
