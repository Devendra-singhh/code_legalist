...
# Non-legal patterns — reject BEFORE calling Groq API (Priority 6)
...
try:
    ...
    if groq_client:
        ...
        print("⚠️  GROQ_API_KEY not set — Groq layer disabled")
except ImportError:
    groq_client = None
    print("⚠️  groq package not installed — Groq layer disabled")

GROQ_MODEL = "llama-3.3-70b-versatile"


def groq_answer(query: str, top_sections: List[Dict]) -> Optional[str]:
    # Guard: no sections or no client
    if groq_client is None or not top_sections:
        return None
    # Priority 6: reject non-legal queries before hitting the API
    q_lower = query.lower()
...
print("=" * 65)
print("CODELEGALIST 3.0 — Initialising")
print("=" * 65)


def load_legal_dataset(repo_id: str, label: str):
    try:
        ds = load_dataset(repo_id, split="train")
        print(f"✅ {label}: {len(ds)} rows | Columns: {ds.column_names}")
        return ds
    except Exception as e:
        print(f"❌ {label}: {e}")
        return None
...
bns_dataset  = load_legal_dataset("dev2703/bns-2023-dataset",  "BNS")
bnss_dataset = load_legal_dataset("dev2703/bnss-2023-dataset", "BNSS")
bsa_dataset  = load_legal_dataset("dev2703/bsa-2023-dataset",  "BSA")
...
print(f"\n✅ {sum(1 for v in DYN.values() if v)}/{len(DYN)} BNS sections resolved")
for k, v in DYN.items():
    if v:
        title = BNS_TITLE.get(v, "?")
        print(f"   DYN[{k}] = Section {v} — {title}")
...
for _sec, _name in _REQUIRED.items():
    assert _sec in BNS_IDX, f"❌ STARTUP FAIL: BNS Section {_sec} ({_name}) not in index."
print("✅ All required BNS sections present")
...
# FIX 1: Robbery interceptor extended with threat+item logic
ROBBERY_KWS = [
    "robbery", "robbed",
...
def is_robbery_query(query: str) -> bool:
    ...
    if any(w in q for w in ROBBERY_KWS):
        return True
    # FIX 1: "someone threatened me and took my phone" etc.
...
# FIX 3: Grievous Hurt keywords expanded
GRIEVOUS_HURT_KWS = [
    ...
    "permanently disfigured",       # FIX 3
    "face disfigured",              # FIX 3
    "acid attack",
...
DIRECT_BNS: Dict[str, str] = {
    ...
    "cheated"              : DYN.get("cheating")            or "318",   # FIX 2
...
print(f"✅ {len(DIRECT_BNS)} direct BNS routes registered")
...
SCENARIO_MAP: Dict[str, str] = {
    ...
    "cheated"          : "cheating fraud deception",    # FIX 2
    "cheated me"       : "cheating fraud deception",    # FIX 2
...
print(f"✅ {len(BNS_SUMMARIES)} BNS summaries mapped")
...
try:
    ...
    _path = os.path.join(_dir, "code_legalist_v3", "datasets", "lawyers.json")
    ...
    if os.path.exists(_path):
        ...
        print("⚠️ lawyers.json not found for chatbot recommendations")
except Exception as _e:
    print(f"Error loading lawyers.json: {_e}")
...
KEYWORD_BOOST_MAP: Dict[str, Tuple[str, Optional[str]]] = {
    ...
    r"trespass|entered.*house|broke.*into"                                        : ("trespass",            DYN.get("trespass")),
...
def get_legal_answer(user_query: str, history: list = None) -> str:
    ...
    # ----------------------------------------------------------------
    # PRIORITY 2 — Collect ALL matching interceptors before returning
    # Single match → clean single response
    # Multiple matches → multi-offence response
    # ----------------------------------------------------------------
    ...
    # Robbery only if dacoity not triggered
    if is_robbery_query(query) and not is_dacoity_query(query):
    ...
    if intercepted:
        # Single match OR definition/punishment → clean single response
        ...
        # Multiple matches (compound scenario) → multi-offence response (Priority 2)
        cit  = CitationTracker()
        ...
        resp += "\n💡 **Next Steps:**\n"
        resp += "• File an FIR at the nearest police station\n"
        resp += "• Preserve all evidence (messages, screenshots, CCTV)\n"
        resp += "• Consult a lawyer — Free: NALSA **15100**\n"
        resp += "💡 *Type \"draft FIR\" for a ready-to-fill FIR template*\n"
        resp += recommend_lawyer_type(query, show=True)
        resp += cit.render()
        return resp + "\n" + LEGAL_DISCLAIMER

    # ----------------------------------------------------------------
    # Direct BNS routing for definition / punishment / general
    # ----------------------------------------------------------------
...
def chat_interface(message: str, history: list) -> str:
    ...
    return get_legal_answer(message, history)


demo = gr.ChatInterface(
    fn=chat_interface,
    title="⚖️ CodeLegalist 3.0 — AI Indian Legal Assistant",
    description=(
        "Complete legal assistance: **BNS** (Offences) + **BNSS** (Procedure) + **BSA** (Evidence) 2023\n\n"
        "Ask questions, describe your situation, or generate legal documents.\n\n"
        "🔍 *Powered by AI + BNS/BNSS/BSA Datasets + Groq (if configured)*"
        " &nbsp;|&nbsp; 📚 *Educational use only*"
    ),
    examples=[
        "What is theft?",         "What is murder?",           "What is dacoity?",
        "What is cheating?",      "What is rape?",             "What is abduction?",
        "What is sexual harassment?", "What is attempt to murder?", "What is extortion?",
        "What is grievous hurt?", "What is robbery?",          "What is criminal intimidation?",
        "Punishment for murder",  "Punishment for theft",      "Punishment for robbery",
        "Section 303",            "Section 309",               "Section 318",
        "How to file an FIR?",    "What is anticipatory bail?","What is police custody?",
        "What is remand?",        "What is a charge sheet?",   "What is zero FIR?",
        "What is electronic evidence?", "Can WhatsApp chats be used as evidence?",
        "Can call recordings be used as evidence?",
        "Someone stole my bike",  "My phone was stolen",       "My wallet is missing",
        "Someone stole my laptop",
        "I was robbed at gunpoint", "Someone threatened and took my phone",
        "Five men robbed my house",
        "I was cheated / duped",  "Someone cheated me",        "Someone scammed me online",
        "Someone blackmailed me", "I received a death threat",
        "Someone hacked my bank account", "OTP fraud happened to me",
        "My Instagram account was hacked", "Telegram scam",
        "Someone created a fake profile of me",
        "Five men entered my house, beat me, stole jewellery, and threatened me",
        "My husband threatens me daily",
        "I got arrested",
        "Draft FIR for theft of my mobile phone",
        "Draft legal notice", "Draft RTI application",
